...
namespace UnityEngine.XR.iOS
{
	public class ARKitRemoteConnection : MonoBehaviour
	{
		...
		void Start () {
			...
			editorConnection.Register (ConnectionMessageIds.screenCaptureRenTextureMsgId, ReceiveRemoteScreenRT);
		}
		...
		void InitializeTextures(UnityARCamera camera)
		{
			...
			yWidth =1280;
			yHeight = 720;
			if (remoteScreenTex == null || remoteScreenTex.width != yWidth 
				|| remoteScreenTex.height != yHeight) {
				if (remoteScreenUVTex) {
					Destroy (remoteScreenTex);
				}
				remoteScreenTex = new Texture2D ( yWidth, yHeight, TextureFormat.RGB24, false);
			}
		...
		void ReceiveRemoteScreenRT (MessageEventArgs mea)
		{
			if (!bTexturesInitialized)
				return;
			...
			UnityARVideo arVideo = Camera.main.GetComponent<UnityARVideo>();
			if (arVideo) {
				arVideo.SetTexure(remoteScreenTex);
			}
			Debug.Log ("asdf");
		}
